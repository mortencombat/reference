 ...
 *
 * `theme.index_categories` is the single source of truth: a post is kept
 * only if at least one of its categories is listed there. Individual posts
 * can be dropped by slug through `theme.exclude_posts`. Removed posts get no
 * page, no search entry and no sitemap entry.
 */
hexo.extend.filter.register('before_generate', function () {
  const Post = hexo.model('Post');
  const allowed = new Set(hexo.theme.config.index_categories || []);
  ...
  Post.forEach((post) => {
    ...
    } else if (!categories.some((name) => allowed.has(name))) {
      remove.push({
        post,
        reason: `not in index_categories (${categories.join(', ') || 'no category'})`
      });
    }
  });
